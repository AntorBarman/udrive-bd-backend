...
const createVehicleSchema = Joi.object({
  ...
  year: Joi.number().integer().min(2000).max(2027).required(),
  vehicle_type: Joi.string().valid('sedan', 'suv', 'hatchback', 'microbus', 'pickup', 'luxury').required(),
  transmission: Joi.string().valid('automatic', 'manual').required(),
  fuel_type: Joi.string().valid('petrol', 'diesel', 'cng', 'hybrid', 'electric').required(),
  seats: Joi.number().integer().min(2).max(15).required(),
  daily_rate: Joi.number().min(500).required(),
  deposit_amount: Joi.number().min(1000).required(),
